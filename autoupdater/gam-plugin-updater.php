...
class GAM_Updater {
	...
	public function init_updates ( $file ) {
		...
		register_activation_hook( $this->plugin_name, [ $this, 'plugin_activation' ], 10 );
		register_deactivation_hook( $this->plugin_name, [ $this, 'plugin_deactivation' ], 10 );
		
		add_filter( 'block_local_requests', '__return_false' );
		add_action( 'admin_init', [ $this, 'admin_init' ] );
		
		include_once( 'gam-plugin-updater-api.php' );
		include_once( 'gam-plugin-updater-key-api.php' );
	}
	...
	public function admin_init () {
		...
		add_action( 'shutdown', [ $this, 'store_errors' ] );
	...
	private function admin_requests () {
		...
		} else if ( ! empty( $_GET[ 'dismiss-' . sanitize_title( $this->plugin_slug ) ] ) ) {
			update_option( $this->plugin_slug . '_hide_key_notice', 1 );
		} else if ( ! empty( $_GET['activated_licence'] ) && $_GET['activated_licence'] === $this->plugin_slug ) {
			$this->add_notice( [ $this, 'activated_key_notice' ] );
		} else if ( ! empty( $_GET['deactivated_licence'] ) && $_GET['deactivated_licence'] === $this->plugin_slug ) {
			$this->add_notice( [ $this, 'deactivated_key_notice' ] );
		} else if ( ! empty( $_GET[ $this->plugin_slug . '_deactivate_licence' ] ) ) {
	...
	private function deactivate_licence_request () {
		...
		wp_redirect( remove_query_arg( [
			'activated_licence',
			$this->plugin_slug . '_deactivate_licence',
		], add_query_arg( 'deactivated_licence', $this->plugin_slug ) ) );
		exit;
	}
	...
	private function activate_licence_request () {
		...
		if ( $this->activate_licence( $licence_key, $email ) ) {
			wp_redirect( remove_query_arg( [
				'deactivated_licence',
				$this->plugin_slug . '_deactivate_licence',
			], add_query_arg( 'activated_licence', $this->plugin_slug ) ) );
			exit;
		} else {
			wp_redirect( remove_query_arg( [
				'activated_licence',
	...
	private function init_key_ui () {
		if ( ! $this->api_key ) {
			add_action( 'admin_print_styles-plugins.php', [ $this, 'styles' ] );
			add_action( 'after_plugin_row', [ $this, 'key_input' ] );
			$this->add_notice( [ $this, 'key_notice' ] );
		} else {
			add_action( 'after_plugin_row_' . $this->plugin_name, [ $this, 'multisite_updates' ], 10, 2 );
			add_filter( 'plugin_action_links_' . $this->plugin_name, [ $this, 'action_links' ] );
		}
		
		$this->add_notice( [ $this, 'error_notices' ] );
	}
	...
	public function load_errors () {
		$this->errors = get_option( $this->plugin_slug . '_errors', [] );
	}
	...
	public function activate_licence ( $licence_key, $email ) {
		try {
			...
			$activate_results = json_decode( GAM_Updater_Key_API::activate( [
				'email'          => $email,
	...
	public function deactivate_licence () {
		$reset = GAM_Updater_Key_API::deactivate( [
			'api_product_id' => $this->plugin_slug,
	...
	public function action_links ( $links ) {
		$links[] = '<a href="' . remove_query_arg( [
				'deactivated_licence',
				'activated_licence',
			], add_query_arg( $this->plugin_slug . '_deactivate_licence', 1 ) ) . '">' . 'Deactivate licence' . '</a>';
	...
	public function styles () {
		if ( ! wp_style_is( 'gam-updater-styles', 'enqueued' ) ) {
			wp_enqueue_style( 'gam-updater-styles', plugins_url( basename( plugin_dir_path( $this->plugin_file ) ), basename( $this->plugin_file ) ) . '/autoupdater/assets/css/backend.css' );
		}
	}
	...
	public function key_input ( $file ) {
		if ( strtolower( basename( dirname( $file ) ) ) === strtolower( $this->plugin_slug ) ) {
			include( 'templates/key-input.php' );
		}
	}
	...
	public function get_plugin_version () {
		$response = GAM_Updater_API::plugin_update_check( [
	...
	public function get_plugin_info () {
		$response = GAM_Updater_API::plugin_information( [
			'plugin_name'    => $this->plugin_name,
			'version'        => $this->plugin_data['Version'],
			'api_product_id' => $this->plugin_slug,
			'licence_key'    => $this->api_key,
	...
	public function handle_errors ( $errors ) {
		...
		} else if ( ! empty( $errors['invalid_request'] ) ) {
			$this->add_error( 'Invalid update request' );
		} else if ( ! empty( $errors['invalid_key'] ) ) {
			$this->add_error( $errors['invalid_key'], 'invalid_key' );
		} else if ( ! empty( $errors['no_activation'] ) ) {
			$this->deactivate_licence();
	...
	public function multisite_updates ( $file, $plugin ) {
		...
		remove_filter( 'pre_set_site_transient_update_plugins', [ $this, 'check_for_updates' ] );
